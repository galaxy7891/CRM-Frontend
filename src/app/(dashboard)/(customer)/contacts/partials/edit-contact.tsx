...
const EditContact: React.FC<FormEditProps> = ({ onClose, contactData }) => {
  const [errorMessage, setErrorMessage] = useState<contactData | null>(null);
  const [contact, setContact] = useState<contactData>({
    id: contactData?.id,
    first_name: contactData?.first_name,
    last_name: contactData?.last_name,
    customerCategory: contactData?.customerCategory,
    job: contactData?.job,
    ...
    subdistrict: contactData?.subdistrict,
    village: contactData?.village,
  ...
  const handleEditContact = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/contacts/${contactData?.id}`,
        contact,
        {
          headers: {
